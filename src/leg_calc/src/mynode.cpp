...
LegControl::LegControl(LegParam_t& leg_param, std::string name)
    ...
    legs_state_sub = this->create_subscription<robot_interfaces::msg::Robot>(
        "legs_status", 10, [this](const robot_interfaces::msg::Robot& msg) {
			RCLCPP_INFO(this->get_logger(),"订阅到最新的电机状态，发布给rviz2使用");
...
void LegControl::Run_Cb() {
    ...
    bool arrivable=false;
        Vector3D temp=leg->CalculateJointPosition(&arrivable);
    //根据期望的笛卡尔坐标系位置计算关节期望角度 if(arrivable==true) std::get<0>(leg_target)=temp;
    ...
    bool arrivable = false;
    Vector3D temp =
        leg->CalculateJointPosition(&arrivable); // 根据期望的笛卡尔坐标系位置计算关节期望角度
    if (arrivable == true)
    ...
    joint_msg.name     = {"joint1", "joint2", "joint3"};
    joint_msg.position = {
        std::get<0>(leg_target)[0], std::get<0>(leg_target)[1], std::get<0>(leg_target)[2]};
    /*if (arrivable)
        RCLCPP_INFO(
            get_logger(), "可以到达:joint1=%lf,joint2=%lf,joint3=%lf", std::get<0>(leg_target)[0],
            std::get<0>(leg_target)[1], std::get<0>(leg_target)[2]);
    else
