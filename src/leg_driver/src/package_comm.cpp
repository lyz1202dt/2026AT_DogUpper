...
PackageComm::PackageComm() {
    ...
    cdc_device->regeiser_recv_cb([this](uint8_t* data, int size) {
        on_recv_cdc(data, size);
        RCLCPP_INFO(rclcpp::get_logger("packageComm"), "接收到CDC数据包");
    });
...
void PackageComm::on_recv_cdc(const uint8_t* data, int size) {
    ...
    if (trans->pack_id)                       // 包ID不为0
    {
        if (last_recv_pack_id<= trans->pack_id) // 如果当前包的ID大于等于之前包的ID，说明此时是全新的传输任务，将本次接收的数据包复制到用户缓冲区新位置
        {
            recv_buffer_index=0;
            // RCLCPP_INFO(
            //     rclcpp::get_logger("packageComm"), "缓冲区索引清零 上一个ID %d，本次ID %d",
            //     last_recv_pack_id, trans->pack_id);
        }
        if (recv_buffer_index + size - sizeof(uint32_t) < 512) {
            std::memcpy(
                recv_buffer + recv_buffer_index, trans->data,
                size - sizeof(uint32_t)); // 将接收的数据包拷贝到用户缓冲区
            recv_buffer_index = recv_buffer_index + size - sizeof(uint32_t);
        }
    } else                                // 包ID为0，说明本次包传输任务已经完成
    {
        // RCLCPP_INFO(
        //     rclcpp::get_logger("packageComm"), "关键字节序(1)%d (60)%d (100)%d ", recv_buffer[0],
        //     recv_buffer[59], recv_buffer[99]);
        std::memcpy(recv_buffer + recv_buffer_index, trans->data, size - sizeof(uint32_t));
        recv_buffer_index = recv_buffer_index + size - sizeof(uint32_t);
        pack_recv_cb(recv_buffer, (int)recv_buffer_index);
        recv_buffer_index = 0;
    }
