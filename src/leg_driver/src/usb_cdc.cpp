...
bool CDCDevice::open(uint16_t vid,uint16_t pid,int interfaces_num)
{
    ...
    //填写异步传输结构体参数
    libusb_fill_bulk_transfer(recv_transfer,handle,EP_IN,reinterpret_cast<uint8_t*>(cdc_rx_buffer),sizeof(cdc_rx_buffer),
    ...
    if(libusb_has_capability(LIBUSB_CAP_HAS_HOTPLUG)){
        int rc=libusb_hotplug_register_callback(ctx,static_cast<libusb_hotplug_event>(LIBUSB_HOTPLUG_EVENT_DEVICE_LEFT|LIBUSB_HOTPLUG_EVENT_DEVICE_ARRIVED), 
...
int CDCDevice::send(void* data,int size,unsigned int time_out)
{
    int actual_size;
